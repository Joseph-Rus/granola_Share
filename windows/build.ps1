# Builds Study Stash for Windows: the app, a zip of it for the installer and auto-update, and
# Study-Stash-Setup.exe for people who download it by hand.
...
if ($Iscc) {
  & $Iscc "/Qp" "/DAppVersion=$Version" "/DSource=$App" "/O$Out" (Join-Path $Here "setup.iss")
  if ($LASTEXITCODE -ne 0) { throw "the installer didn't build" }
} else {
  Write-Host "Inno Setup isn't installed, so there's no Study-Stash-Setup.exe (choco install innosetup)."
}
