...
ACCOUNT_JSON = {"email": "alex@example.com",
                "active_workspace": {"id": "ws-77", "display_name": "Alex's workspace"},
...
def test_normalize_account_with_the_live_payload():
    ...
    assert info["workspace"] == "Alex's workspace" and info["workspace_id"] == "ws-77"
...
def test_account_label():
    assert account_label(normalize_account(ACCOUNT_JSON)) == "alex@example.com · Alex's workspace"
...
def test_client_get_account_info(tmp_path):
    ...
    assert info["email"] == "alex@example.com" and info["workspace"] == "Alex's workspace"
...
<meeting id="1f0c2a7e-3b4d-4e5f-8a6b-7c8d9e0f1a2b" title="Cell biology — membranes with Dr. Lee" date="Sep 15, 2026 2:20 PM PDT" captured_by_me="true" listed_as_participant="true" is_workspace_visible="false">
    <known_participants>
    alex rivera (note creator) &lt;someone@example.com&gt;
    </known_participants>
  </meeting>

<meeting id="2a1b3c4d-5e6f-4a7b-8c9d-0e1f2a3b4c5d" title="sep 14 calculus" date="Sep 14, 2026 8:15 AM PDT" captured_by_me="true" listed_as_participant="true" is_workspace_visible="false">
    <known_participants>
    alex rivera (note creator) &lt;someone@example.com&gt;
    </known_participants>
  </meeting>
...
def test_list_payload_survives_access_notice_and_prose_preamble():
    ...
    assert first.id == "1f0c2a7e-3b4d-4e5f-8a6b-7c8d9e0f1a2b"
    assert first.title.startswith("Cell biology")
    assert first.date == "2026-09-15T14:20:00"        # 2:20 PM PDT, zone dropped, wall time kept
    assert first.owner == "alex rivera"              # the (note creator) participant
    assert first.attendees == ["alex rivera"]
