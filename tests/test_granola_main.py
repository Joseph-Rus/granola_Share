...
def test_extract_meetings_parses_granola_xml():
    ...
    assert normalize_meeting(first).date == "2026-09-15T14:20:00"  # PM -> ISO (done when normalizing)
    assert "3 < 5 items" in first["summary"]                       # summary body unescaped
    assert normalize_meeting(first).attendees == ["alex rivera"]  # email + (note creator) stripped
