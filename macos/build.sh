#!/bin/sh
# Builds "Granola Share.app" (Apple silicon + Intel), a zip of it for the installer, and a DMG.
...
for arch in arm64 x86_64; do
  xcrun swiftc -O -swift-version 5 -target "$arch-apple-macos12.0" -framework AppKit -framework WebKit \
    -o "$OUT/app-$arch" "$HERE/GranolaShare.swift"
done
...
(cd "$OUT" && ditto -c -k --keepParent "$NAME.app" "Granola-Share-mac.zip")

STAGE="$OUT/dmg"
mkdir -p "$STAGE"
cp -R "$APP" "$STAGE/"
ln -s /Applications "$STAGE/Applications"
hdiutil create -quiet -volname "$NAME" -srcfolder "$STAGE" -ov -format UDZO "$OUT/Granola-Share.dmg"
rm -rf "$STAGE"
